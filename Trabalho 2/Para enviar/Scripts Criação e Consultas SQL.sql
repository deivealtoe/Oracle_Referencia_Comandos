...
Deivisson Altoé Ferreira
Lucas de Sena Bastos
André de Almeida Lopes de Souza
Marco Antonio Gonçalves Silva
Marcio Marques de Freitas
Marco Antônio Patrocinio
...
σ Nome = “A” (Cliente)
σ (Telefone = “27” OR Telefone = “28” (Telefone))
σ Cidade = “Serra” (Endereço)
σ PrecoDiaria < 600,00 (Carro)
σ Cor = “Prata” (Carro)
σ Modelo = “Ret” (Modelo)
...
RN 01 - O telefone pode pertencer a no mínimo e no máximo um cliente.
RN 02 - O cliente pode possuir um telefone ou vários.
RN 03 - O endereço pode estar vinculado a no mínimo um cliente e no máximo vários clientes.
RN 04 - O cliente pode estar vinculado a no mínimo e no máximo um endereço.
RN 05 - O cliente pode alugar vários ou nenhum carro.
RN 06 - O carro pode ser alugado por nenhum ou vários clientes.
RN 07 - O carro pode conter no mínimo e no máximo um modelo.
RN 08 - O modelo pode estar presente em no mínimo um carro e no máximo vários carros.
*/
...
SELECT * FROM aluguel alu INNER JOIN cliente cli ON alu.codcliente = cli.codcliente INNER JOIN carro car ON alu.codcarro = car.codcarro;
SELECT * FROM telefone INNER JOIN cliente ON telefone.codcliente = cliente.codcliente;
SELECT * FROM cliente;

SELECT * FROM cliente WHERE nome LIKE '%A%';
SELECT * FROM telefone WHERE TO_CHAR(telefone) LIKE '%27%' OR TO_CHAR(telefone) LIKE '%28%';
SELECT * FROM endereco WHERE cidade = 'Serra';
SELECT * FROM carro WHERE precodiaria < 600;
SELECT * FROM carro WHERE cor = 'Preto';
SELECT * FROM modelo WHERE modelo = 'Ret';

SELECT estado, cidade, bairro FROM endereco;
SELECT nome, cnh FROM cliente;
SELECT c.nome, e.estado, e.cidade, e.bairro FROM cliente c INNER JOIN endereco e ON c.codendereco = e.codendereco;
SELECT c.placa, c.precodiaria, c.ano, m.modelo FROM carro c INNER JOIN modelo m ON c.codmodelo = m.codmodelo;
SELECT cli.nome, car.placa, alu.datalocacao, alu.datadevolucao FROM aluguel alu INNER JOIN cliente cli ON alu.codcliente = cli.codcliente INNER JOIN carro car ON alu.codcarro = car.codcarro;
SELECT cliente.nome, telefone.telefone FROM telefone INNER JOIN cliente ON telefone.codcliente = cliente.codcliente;
